  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-24 md:py-36 lg:px-8">
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-border bg-surface-2/50">
                  <th className="px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.08em] text-muted-light">Module</th>
                  <th className="px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.08em] text-muted-light">Responsibility</th>
                </tr>
              </thead>
              <tbody>
                {controlPlaneModules.map((m) => (
                  <tr key={m.module} className="border-b border-border-light last:border-0">
                    <td className="px-4 py-3 text-sm font-medium text-foreground whitespace-nowrap">{m.module}</td>
                    <td className="px-4 py-3 text-sm text-muted">{m.desc}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <AnimateOnScroll className="mt-24">
        ...
        <div className="mt-8 overflow-hidden rounded-2xl border border-border">
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-border bg-surface-2/50">
                <th className="px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.08em] text-muted-light">Property</th>
                <th className="px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.08em] text-muted-light">Implementation</th>
              </tr>
            </thead>
            <tbody>
              {infraProperties.map((p) => (
                <tr key={p.property} className="border-b border-border-light last:border-0">
                  <td className="px-4 py-3 text-sm font-medium text-foreground whitespace-nowrap">{p.property}</td>
                  <td className="px-4 py-3 text-sm text-muted">{p.impl}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <ContinueReading
        href="/developer"
        label="Developer"
        description="API endpoints, batch screening, webhooks, and integration patterns."
      />
    </div>
  )
}
