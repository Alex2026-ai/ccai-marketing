...
Every decision generates a DecisionEventPackage containing resolution lineage, rule bindings, watchlist snapshot references, and confidence scores. Evidence is not an add-on — it is the primary output.
...
Entities pass through up to five resolution layers — from deterministic matching to vector similarity to semantic reasoning to human review. Each layer is independently auditable. Cost scales with complexity, not volume.
...
| Regional data residency     | EU and US deployment with enforced data locality                 |
| Operator dashboard          | Web interface for batch management, case triage, and diagnostics |
...
Screen vessel owners, operators, and beneficial ownership chains against OFAC SDN, EU consolidated, and UN sanctions lists. Monitor ongoing exposure across fleet portfolios.

### Insurance

Screen policyholders, counterparties, and reinsurance chains at underwriting and renewal. Batch-process portfolio-scale entity lists with full evidence for regulatory reporting.

### Enterprise Platforms

Embed CCAI as a screening layer within existing compliance workflows. API-first integration for platforms that need sanctions screening as infrastructure, not a standalone product.
